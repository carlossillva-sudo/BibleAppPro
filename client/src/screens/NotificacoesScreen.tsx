...
const NotificacoesScreen: React.FC = () => {
  ...
  useEffect(() => {
    updateSchedule();
  }, [notificationsEnabled, dailyVerseEnabled, readingReminderEnabled, selectedTimeDailyVerse, selectedTimeReadingReminder]);
  ...
  return (
    <div className="p-5 max-w-3xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <section className="bg-card border border-foreground/5 rounded-[32px] p-6 shadow-sm space-y-4">
        ...
        <NotificationToggleItem label="Ativar" value={dailyVerseEnabled} onChange={toggleDailyVerse} disabled={!notificationsEnabled} />
        <TimeSelectorItem label="Horário" time={selectedTimeDailyVerse} onChange={setDailyTime} disabled={!dailyVerseEnabled || !notificationsEnabled} />
      ...
      <section className="bg-card border border-foreground/5 rounded-[32px] p-6 shadow-sm space-y-4">
        ...
        <NotificationToggleItem label="Ativar" value={readingReminderEnabled} onChange={toggleReadingReminder} disabled={!notificationsEnabled} />
        <TimeSelectorItem label="Horário" time={selectedTimeReadingReminder} onChange={setReadingTime} disabled={!readingReminderEnabled || !notificationsEnabled} />
      ...
      <section className="bg-card border border-foreground/5 rounded-[32px] p-6 shadow-sm space-y-4">
        ...
        <NotificationToggleItem label="Som" value={soundEnabled} onChange={toggleSound} disabled={!notificationsEnabled} />
        <NotificationToggleItem label="Vibração" value={vibrationEnabled} onChange={toggleVibration} disabled={!notificationsEnabled} />
      </section>

      <section className="bg-card border border-foreground/5 rounded-[32px] p-6 shadow-sm space-y-4">
        <SectionHeader title="Horário de Silêncio" description="Não receber notificações durante este período" />
        <NotificationToggleItem label="Ativar" value={quietHoursStart !== ''} onChange={(v) => setQuietHours(v ? '22:00' : '', v ? '07:00' : '')} />
        <TimeSelectorItem label="Início" time={quietHoursStart} onChange={(t) => setQuietHours(t, quietHoursEnd)} />
        <TimeSelectorItem label="Fim" time={quietHoursEnd} onChange={(t) => setQuietHours(quietHoursStart, t)} />
      </section>
      ...
      <section className="bg-card border border-foreground/5 rounded-[32px] p-6 shadow-sm space-y-3">
        ...
        <p>Agendadas:</p>
        {scheduled.map(n => <p key={n.id}>{n.id} @ {n.time}</p>)}
        <button
