...
export const PersonalizationPage: React.FC = () => {
  ...
  const handleReset = () => {
    if (confirm('Tem certeza que deseja restaurar as preferências padrões?')) {
      resetDefaults();
    }
  };
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-12 pb-32 animate-in fade-in duration-500">
      ...
      <div className="grid gap-8">
        ...
        <section className="bg-card border border-foreground/5 rounded-[32px] p-6 md:p-8 shadow-sm">
          <div className="mb-6 space-y-1">
            <h2 className="font-black text-xl flex items-center gap-3">
              <Monitor className="h-5 w-5 text-primary" /> Modo de Aparência
            </h2>
            <p className="text-sm text-muted-foreground">
              O modo "Sistema" acompanha a preferência atual do seu dispositivo.
            </p>
          </div>
        ...
        <section className="bg-card border border-foreground/5 rounded-[32px] p-6 md:p-8 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
            <div className="space-y-1">
              <h2 className="font-black text-xl flex items-center gap-3">
                <Palette className="h-5 w-5 text-primary" /> Temas Premium
              </h2>
              <p className="text-sm text-muted-foreground">
                Selecione uma paleta de cores para customizar a interface.
              </p>
            </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
            {filteredThemes.map((t) => (
              <button
                ...
                className={cn('flex flex-col items-center gap-3 transition-all relative group')}
              >
                <div
                  className={cn(
                    ...
                    theme === t.id
                      ...
                      : 'border-foreground/10 group-hover:scale-105'
                  )}
        ...
        <section className="bg-card border border-foreground/5 rounded-[32px] p-6 md:p-8 shadow-sm space-y-10">
          <div className="space-y-1">
            <h2 className="font-black text-xl flex items-center gap-3">
              <Type className="h-5 w-5 text-primary" /> Tipografia & Layout
            </h2>
            <p className="text-sm text-muted-foreground">
              Ajuste o texto para a leitura mais confortável possível.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <span className="text-xs font-black text-muted-foreground uppercase tracking-widest block mb-2">
                  Família da Fonte
                </span>
              ...
              {widthOptions.map((w) => (
                <button
                  ...
                  className={cn(
                    ...
                    readingWidth === w.value
                      ? 'bg-primary/5 border-primary text-primary shadow-sm'
                      : 'border-foreground/5 bg-muted/20 text-muted-foreground hover:bg-muted'
                  )}
                >
          ...
          <div className="p-6 md:p-8 rounded-[24px] bg-background border border-border shadow-inner relative mt-8">
            <span className="absolute top-0 right-8 px-3 py-1 bg-primary text-white text-[9px] font-black uppercase tracking-widest rounded-b-lg shadow-sm">
              Preview
            </span>
            <p
            ...
            >
              ...
              <span className={cn(boldVerses && 'font-bold')}>
                O Senhor é o meu pastor; de nada me terei falta. Em verdes pastagens me faz
                repousar.
              </span>
            </p>
          </div>
        ...
        <section className="bg-card border border-foreground/5 rounded-[32px] p-6 md:p-8 shadow-sm space-y-6">
          <div className="space-y-1">
            <h2 className="font-black text-xl flex items-center gap-3">
              <LayoutTemplate className="h-5 w-5 text-primary" /> Comportamento do Leitor
            </h2>
            <p className="text-sm text-muted-foreground">
              Personalize as ferramentas na interface de leitura.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              {
                id: 'numbers',
                title: 'Números dos Versículos',
                desc: 'Exibir os numerais antes de cada verso',
                icon: Eye,
                state: showVerseNumbers,
                setter: setShowVerseNumbers,
              },
              {
                id: 'bold',
                title: 'Versículos em Negrito',
                desc: 'Destacar fortemente os textos',
                icon: Type,
                state: boldVerses,
                setter: setBoldVerses,
              },
              {
                id: 'smooth',
                title: 'Transições Suaves',
                desc: 'Animações fluidas entre páginas do leitor',
                icon: BookOpen,
                state: smoothTransitions,
                setter: setSmoothTransitions,
              },
            ].map((item) => (
              <button
                key={item.id}
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={cn(
                      'p-2 rounded-xl',
                      item.state ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'
                    )}
                  >
                    <item.icon className="h-4 w-4" />
                  </div>
                  <div>
                    <p className={cn('text-sm font-bold', item.state ? 'text-primary' : '')}>
                      {item.title}
                    </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div className="space-y-3 p-5 rounded-2xl bg-muted/10 border border-border">
              <span className="text-xs font-black text-foreground uppercase tracking-widest block">
                Espaçamento entre Versículos
              </span>
        ...
        {/* 5. ÍCONES DA BARRA LATERAL */}
        <section className="bg-card border border-foreground/5 rounded-[32px] p-6 md:p-8 shadow-sm space-y-6">
          <div className="space-y-1">
            <h2 className="font-black text-xl flex items-center gap-3">
              <LayoutGrid className="h-5 w-5 text-primary" /> Ícones da Barra Lateral
            </h2>
            <p className="text-sm text-muted-foreground">
              Escolha as cores dos ícones de navegação.
            </p>
          </div>
            ...
            {[
              {
                ...
                label: 'Do Tema',
                icon: Palette,
                desc: 'Cor do tema atual',
                color: 'bg-foreground',
              },
              { value: 'blue', label: 'Azul', icon: Sun, desc: '#2563eb', color: 'bg-blue-600' },
              { value: 'green', label: 'Verde', icon: Sun, desc: '#16a34a', color: 'bg-green-600' },
              {
                value: 'purple',
                label: 'Roxo',
                icon: Sun,
              ...
              { value: 'red', label: 'Vermelho', icon: Sun, desc: '#dc2626', color: 'bg-red-600' },
              {
                value: 'gradient',
                label: 'Gradiente',
                icon: Sun,
              ...
              <button
                key={opt.value}
                onClick={() => setSidebarIconColor(opt.value as any)}
                className={cn(
                  'flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all',
                  sidebarIconColor === opt.value
                    ? 'border-primary bg-primary/5 shadow-md'
                    : 'border-transparent bg-muted/30 hover:bg-muted'
                )}
              >
                <div
                  className={cn('w-8 h-8 rounded-lg flex items-center justify-center', opt.color)}
                >
                  <opt.icon
                    className={cn(
                      'h-4 w-4',
                      opt.value === 'theme' ? 'text-background' : 'text-white'
                    )}
                  />
                </div>
                <span
                  className={cn(
                    'text-xs font-bold',
                    sidebarIconColor === opt.value ? 'text-primary' : 'text-muted-foreground'
                  )}
                >
                  {opt.label}
                </span>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between p-4 px-6 md:p-6 bg-primary/5 border border-primary/20 rounded-3xl mt-12 mb-8">
        <div className="text-center md:text-left mb-4 md:mb-0">
          <p className="text-sm font-bold text-foreground">
            As preferências são aplicadas instantaneamente.
          </p>
          <p className="text-[11px] font-medium text-muted-foreground">
            Salvas automaticamente no seu dispositivo.
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            ...
            variant="outline"
            size="lg"
            className="px-4 rounded-2xl gap-2"
          >
          ...
          <Button
            onClick={handleSave}
            size="lg"
            className="px-6 rounded-2xl gap-2 font-black shadow-lg shadow-primary/20"
          >
            {saved ? (
              <>
              ...
              <>
                <Save className="h-5 w-5" /> Concluído
              </>
            )}
          </Button>
        </div>
