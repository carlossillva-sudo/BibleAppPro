...
import { Crown, Palette, ChevronRight, LogOut, Info, Cloud } from 'lucide-react';
  ...
  const menuItems = [
    {
      ...
      desc: `Tema: ${theme === 'light' ? 'Claro' : theme === 'dark' ? 'Escuro' : 'Sépia'}`,
      path: '/personalization',
    },
    {
      icon: Cloud,
      label: 'Integração Google',
      desc: 'Backup Drive e lembretes',
      ...
      <section className="bg-card border rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-accent/30 rounded-xl">
            ...
            <button
              onClick={() => toggleNotification('lembrete')}
              className={`w-11 h-6 rounded-full transition-colors relative ${notifications.lembrete ? 'bg-primary' : 'bg-muted'}`}
            >
              <div
                className={`absolute top-1 h-4 w-4 bg-white rounded-full shadow transition-all ${notifications.lembrete ? 'right-1' : 'left-1'}`}
              />
            </button>
          </div>
          <div className="flex items-center justify-between p-3 bg-accent/30 rounded-xl">
            ...
            <button
              onClick={() => toggleNotification('versiculo')}
              className={`w-11 h-6 rounded-full transition-colors relative ${notifications.versiculo ? 'bg-primary' : 'bg-muted'}`}
            >
              <div
                className={`absolute top-1 h-4 w-4 bg-white rounded-full shadow transition-all ${notifications.versiculo ? 'right-1' : 'left-1'}`}
              />
            </button>
          </div>
        </div>
