...
import { Check as CheckIcon, ChevronDown as ChevronDownIcon, BookCopy as BookCopyIcon, Info as InfoIcon } from 'lucide-react';
        ...
        <div className={cn("relative", className)} ref={dropdownRef}>
            ...
            >
                ...
                <div className="flex flex-col items-start leading-tight">
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Versão</span>
                    <div className="flex items-center gap-1">
                        <span className="text-sm font-black tracking-tight">{bibleVersion}</span>
                    ...
                    >
                        ...
                        <div className="p-1.5 max-h-[300px] overflow-y-auto custom-scrollbar">
                            {isLoading ? (
                                <div className="p-3 space-y-2">
                                    {[1, 2, 3].map(i => <div key={i} className="h-10 bg-muted/50 rounded-xl animate-pulse" />)}
                                </div>
                            ) : (
                                versions?.map(v => (
                                    <button
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            <span className="text-sm font-black">{v.id}</span>
                                            <span className="text-[10px] font-medium opacity-60 truncate max-w-[180px]">{v.name}</span>
                                        </div>
                                        {bibleVersion === v.id && (
                        ...
                        <div className="px-4 py-2 bg-slate-50 dark:bg-zinc-900/50 border-t border-slate-100 dark:border-zinc-800">
                            <p className="text-[9px] text-center font-bold text-muted-foreground italic">
                                "{versions?.find(v => v.id === bibleVersion)?.name || bibleVersion}" selecionada
                            </p>
                        </div>
                    </motion.div>
                )}
